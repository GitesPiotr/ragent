...
function linia(provider, id, label, temperaturaTak) {
  const temp = temperaturaTak
    ? "temperatura: tak"
    : "temperatura: NIE (model sam dobiera losowość)";
  return `- ${id} (${opis(provider, label)}) — ${temp}`;
}

// Buduje tekst listy modeli do promptu mentora.
//
//   grupy — [{ provider, modele: [{id,label}] }] w dowolnej kolejnosci
...
export function dopasujModel(katalog, id) {
  ...
  if (dokladne) return { provider: dokladne.provider, id: dokladne.id };

  const luzny = znormalizujId(szukany);
  const tolerancyjne = lista.find((m) => znormalizujId(m.id) === luzny);
  if (tolerancyjne) return { provider: tolerancyjne.provider, id: tolerancyjne.id };
...
export function tekstDostepnychModeli(grupy, czyTemperatura, ollamaNiedostepna) {
  const wg = new Map(
    (Array.isArray(grupy) ? grupy : []).map((g) => [g.provider, g.modele || []]),
  );
  const lines = [];

  for (const provider of KOLEJNOSC) {
    const modele = wg.get(provider) || [];
    ...
    for (const m of modele) {
      lines.push(linia(provider, m.id, m.label, czyTemperatura(provider, m.id)));
    }
  }
