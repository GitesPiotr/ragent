...
import { czyModelMentoraDozwolony } from "@/lib/settings/dopuszczoneModele";
...
// Inaczej: wartość efektywna z env/stałej.
//
// WALIDACJA ZOSTAŁA, ZMIENIŁO SIĘ TYLKO ŹRÓDŁO LISTY. To nadal jest ostatni
// strażnik: `mentorModel` przychodzi z localStorage przeglądarki, czyli z pola,
// które każdy może podmienić. Klient nie ma tu nic do powiedzenia.
//
// `dopuszczone` podaje wołający (trasa mentora, z bazy). Gdy go NIE poda —
// null, czyli „nie wiem" — sprawdzamy tylko, czy to niepusty napis. Serwer bez
// listy nie ma względem czego orzekać, a odrzucanie wszystkiego zamieniłoby
// każdą awarię odczytu bazy w cichy powrót do modelu domyślnego.
//
// Dostawca mentora NIE jest tu parametrem i nie ma być — uzasadnienie
// (structured output w trybie prowadzenia) w lib/settings/dopuszczoneModele.js.
export function resolveMentorModel(raw, dopuszczone = null) {
  if (czyModelMentoraDozwolony(raw, dopuszczone)) return raw;
  return MENTOR_MODEL;
}
