...
test('separator klucza jest bajtem zerowym — i ma nim pozostac', () => {
  // Dwie rzeczy naraz, obie nadepniete:
  //
  // 1. SEPARATOR MA BYC NIEDRUKOWALNY. Dwukropek by nie wystarczyl —
  //    identyfikatory OpenRoutera go zawieraja ("ling-3.0-flash:free"),
  //    wiec pary daloby sie skleic do tego samego napisu na dwa sposoby.
  assert.equal(kluczModelu('a', 'b'), 'a' + String.fromCharCode(0) + 'b');
  assert.notEqual(
    kluczModelu('openrouter', 'x:free'),
    kluczModelu('openrouter:x', 'free'),
  );

  // 2. W ZRODLE MA STAC SEKWENCJA UCIECZKI, NIE ZNAK. Pierwsza wersja miala
  //    tam dosłowny bajt zerowy: dzialala, testy przechodzily, a git uznal
  //    caly plik za binarny — bez diffow i bez blame. Tego nie widac
  //    w przegladzie kodu, wiec pilnuje tego test, a nie czyjes oko.
  const zrodlo = readFileSync(new URL('./modeleKonta.js', import.meta.url));
  assert.equal(zrodlo.indexOf(0), -1, 'plik zrodlowy zawiera dosłowny bajt zerowy');
});
