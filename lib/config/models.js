...
//
//  Na pokazie nie ma Ollamy, więc każdy wybór modelu lokalnego kończy się
//  błędem połączenia. Ten przełącznik chowa je we WSZYSTKICH czterech
//  miejscach, w których dawało się je wybrać.
//
//  DLACZEGO `PROVIDERS` ZOSTAJE NIETKNIĘTE, a filtr jest osobny:
//  ta tablica NIE jest tylko listą do wyboru. Czyta ją także
//   • app/api/settings/models/route.js:139 — walidacja zapisu allowed_models;
//     usunięcie „ollama" odrzucałoby zapis ustawień kontu, które ma
//     dopuszczony choć jeden model lokalny, i to CICHO, jako „nieznany
//     dostawca",
//   • ModelSection.js:258 i app/ustawienia/page.js:350 — podpisy istniejących
//     wyborów; bez wpisu pokazałyby surowe „ollama" zamiast nazwy.
//  Ukrywamy więc możliwość WYBRANIA, nie wiedzę o dostawcy.
//
//  CZTERY MIEJSCA, BO TAKA JEST PRAWDA O TYM KODZIE — jedno źródło ich nie
//  obsługuje. Każde ma własną listę i każde filtruje tym samym przełącznikiem:
//   1. components/creator/sections/ModelSection.js  (kreator agenta),
//   2. components/chats/RunnerPicker.js             (wybór przy rozmowie),
//   3. app/ustawienia/_modele/ModeleJezykowe.js     (Ustawienia),
//   4. app/kreator-rag/kolekcje/page.js             (wektory przy kolekcji,
//      przez lib/config/modeleEmbeddingow.js).
//
//  Przywrócenie: ustawić na true. Nic poza tym.
// =============================================================================
export const POKAZ_DOSTAWCE_LOKALNA = false;

// =============================================================================
//  OPENAI — OSOBNA STAŁA, ŚWIADOMIE NIE TA SAMA
//
//  Powód ukrycia jest inny: OpenAI nie jest dostawcą lokalnym, tylko wpisanym
//  w aplikację BEZ KLUCZA. Wszystkie jego modele mają `verified: false`
//  (patrz MODELS_BY_PROVIDER niżej), a ekran Połączeń pokazywał przy nim
//  „Brak klucza OPENAI_API_KEY" — na pokazie czyta się to jak usterka.
//
//  DLACZEGO NIE JEDNA WSPÓLNA LISTA UKRYTYCH: różnią się CZASEM POWROTU.
//  OpenAI wraca w chwili, gdy pojawi się klucz — jedna zmienna środowiskowa.
//  Ollama wraca dopiero, gdy ktoś ją zainstaluje i pobierze model. Wspólny
//  przełącznik wymuszałby przywracanie obu naraz, czyli pokazanie dostawcy,
//  który dalej nie działa.
//
//  ============================================================================
//  FILTR IDZIE PO IDENTYFIKATORZE DOSTAWCY, NIGDY PO TEKŚCIE W NAZWIE MODELU.
//
//  To jest warunek poprawności całej demonstracji, nie szczegół. Model
//  `openai/gpt-5.6-terra` należy do dostawcy `openrouter` i jest domyślnym
//  modelem agenta na koncie pokazowym. Filtr porównuje `p.id !== "openai"`,
//  czyli identyfikator DOSTAWCY — wpis `{ id: "openrouter" }` nie jest tym
//  wpisem i zostaje nietknięty razem z całym swoim katalogiem.
//
//  Modele konta filtruje modeleKonta() przez kolumnę `provider` w
//  allowed_models (dopuszczoneModele.js), a nie przez treść `model_id`.
//  Nigdzie w tym kodzie nie ma porównania w rodzaju
//  `model_id.startsWith("openai")` — i nie wolno go dopisać.
//  ============================================================================
//
//  Przywrócenie: ustawić na true. Nic poza tym.
// =============================================================================
export const POKAZ_DOSTAWCE_OPENAI = false;

// Jeden filtr, dwa niezależne warunki. `PROVIDERS` zostaje pełne — czyta je
// walidacja zapisu i podpisy istniejących wyborów (uzasadnienie wyżej).
export const PROVIDERS_WIDOCZNI = PROVIDERS.filter(
  (p) =>
    (POKAZ_DOSTAWCE_LOKALNA || p.id !== "ollama") &&
    (POKAZ_DOSTAWCE_OPENAI || p.id !== "openai"),
);
