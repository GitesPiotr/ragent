...
test('etykieta narzędzi mówi wprost, że to deklaracja', () => {
  ...
  assert.equal(etykietaNarzedzi({ deklarujeTools: false }), 'bez deklaracji');
  // Rdzeń „deklar" ma trafić przed oczy użytkownika po OBU stronach, a nie
  // zostać w komentarzu — stąd ten test na samą treść. Strona negatywna mówi
  // o braku INFORMACJI („bez deklaracji"), nie o braku możliwości: 66 modeli
  // nie ma tej flagi, a openrouter/fusion mimo to wywołał narzędzia poprawnie.
  assert.ok(etykietaNarzedzi({ deklarujeTools: true }).includes('deklar'));
  assert.ok(etykietaNarzedzi({ deklarujeTools: false }).includes('deklar'));
  assert.doesNotMatch(etykietaNarzedzi({ deklarujeTools: false }), /nie obsługuje|bez obsługi/);
});
