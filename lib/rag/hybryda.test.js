...
// --- IDENTYFIKATOR ZE ZNAKIEM ŁĄCZĄCYM -------------------------------------------
//
// TE PRZYPADKI PILNUJĄ USTERKI, KTÓRA JUŻ RAZ WYSTĄPIŁA i której objawem było
// TWARDE ZERO, a nie gorszy wynik: „P-01" oddawało `01`, a lexemu `'01'` w bazie
// nie ma — parser Postgresa robi z „P-01" dwa lexemy `'p'` i `'-01'`. Ścieżka
// tekstowa nie znajdowała nic, reguła negatywna orzekała „nie ma" i odrzucała
// wektory, które miały właściwy fragment ze score 0,6882.
//
// Oczekiwana wartość to CAŁY NAPIS IDENTYFIKATORA — to jest sedno poprawki.
// Cięcie na lexemy należy do Postgresa, po obu stronach tak samo.
test('identyfikator z myślnikiem zostaje w całości', () => {
  // Zmierzone na kolekcji z dokumentacją KMX-410 (rag_search_chunks_text):
  //   p_query="P-01" -> 5 trafień      p_query="01"      -> 0
  //   p_query="A-03" -> 5 trafień      p_query="03"      -> 0
  //   p_query="45-0002" -> 1 trafienie p_query="45 0002" -> 0
  //   p_query="KMX-410" -> 5 trafień   p_query="410"     -> 0
  assert.deepEqual(identyfikatoryZapytania('P-01'), ['P-01']);
  assert.deepEqual(identyfikatoryZapytania('A-03'), ['A-03']);
  assert.deepEqual(identyfikatoryZapytania('45-0002'), ['45-0002']);
  assert.deepEqual(identyfikatoryZapytania('KMX-410'), ['KMX-410']);
  assert.deepEqual(identyfikatoryZapytania('RS-485'), ['RS-485']);
});

test('identyfikator z kropką zostaje w całości', () => {
  // Ta sama usterka, inny znak. Zmierzone: p_query="6.1" -> „6.1 Histereza",
  // a p_query="6 1" -> dwa CZYM INNYM fragmenty („3.2 Montaż", „3.6 Zasilanie").
  assert.deepEqual(identyfikatoryZapytania('6.1'), ['6.1']);
  assert.deepEqual(identyfikatoryZapytania('rozdział 15.5'), ['15.5']);
});

test('zapytanie mieszane: słowo obok identyfikatora', () => {
  assert.deepEqual(identyfikatoryZapytania('parametr P-01'), ['P-01']);
  assert.deepEqual(identyfikatoryZapytania('co robi parametr P-01 w sterowniku'), ['P-01']);
  assert.deepEqual(identyfikatoryZapytania('alarm A-03 i alarm A-05'), ['A-03', 'A-05']);
});

test('wielkość liter nie tworzy drugiego identyfikatora', () => {
  // Słownik `simple` i tak sprowadza do małych liter po stronie Postgresa —
  // tu pilnujemy tylko tego, żeby nie poszły DWA tokeny w AND-zie.
  assert.deepEqual(identyfikatoryZapytania('P-01 oraz p-01'), ['P-01']);
});

test('znak łączący na końcu NIE wchodzi do identyfikatora', () => {
  // Kropka kończąca zdanie i myślnik przed spacją muszą zostać na zewnątrz,
  // inaczej do tsquery poszedłby napis, którego w dokumencie nie ma.
  assert.deepEqual(identyfikatoryZapytania('sprawdź P-01.'), ['P-01']);
  assert.deepEqual(identyfikatoryZapytania('parametr P-01 - histereza'), ['P-01']);
});

// --- REGRESJA: identyfikatory BEZ znaku łączącego --------------------------------
//
// Te zapytania działały przed poprawką i mają działać dalej. „§ 45" jest tu
// najważniejsze: to jest przypadek z rundy 4 integracji, naprawiony migracją 018,
// który wchodzi ścieżką tekstową PONIŻEJ progu wektorowego (score 0,4424).
test('identyfikator bez znaku łączącego — bez zmian', () => {
  assert.deepEqual(identyfikatoryZapytania('§ 45'), ['45']);
  assert.deepEqual(identyfikatoryZapytania('art. 36'), ['36']);
  assert.deepEqual(identyfikatoryZapytania('art. 33'), ['33']);
  assert.deepEqual(identyfikatoryZapytania('art. 5 ust. 1'), ['5', '1']);
  assert.deepEqual(identyfikatoryZapytania('art. 9999'), ['9999']);
});

test('zapytanie opisowe nadal NIE uruchamia ścieżki tekstowej', () => {
  // Zapytanie z pomiaru (d): ma zwracać „6.1 Histereza" ze score 0,6882 przez
  // same wektory. Pusta lista znaczy `tekstowe: null`, czyli reguła negatywna
  // nie ma prawa się odezwać.
  assert.deepEqual(identyfikatoryZapytania('histereza regulacji temperatury'), []);
  assert.deepEqual(identyfikatoryZapytania('jak skasować alarm'), []);
});
