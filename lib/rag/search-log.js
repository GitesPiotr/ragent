...
export function zapiszWyszukiwanie(wpis, deps = {}) {
  ...
  const wiersz = {
    ...
    // owner_ref ZNIKNĘŁO — kolumnę kasuje supabase/016_rls_rag.sql (KROK 2).
    // Właściciela wpisuje teraz baza: owner_id ma default auth.uid(), a zapis idzie
    // klientem z sesją użytkownika, więc nie ma tu czego podawać jawnie. Gdyby ta
    // linia została, PostgREST odrzucałby KAŻDY wpis do dziennika ("Could not find
    // the 'owner_ref' column") — po cichu, bo osłony niżej zamieniają błąd zapisu
    // dziennika na ostrzeżenie w logach, żeby nigdy nie wywrócił wyszukiwania.
  };
