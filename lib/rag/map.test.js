...
// =============================================================================
//  PRÓG NIE MOŻE ODPALAĆ SIĘ CO PARTIĘ (12.9)
//
//  Przeliczenie rusza WSZYSTKIE współrzędne. Gdyby zachodziło po każdej partii,
//  punkty przesuwałyby się co kilkanaście sekund przez całe indeksowanie — czyli
//  dokładnie ten ciągły ruch, przed którym broni 12.9. Chroni przed tym para
//  warunków: refreshProjectionAfterIndexing jest wołane TYLKO przy `finished`
//  (documents.js), a próg liczy się od bazy, która po przeliczeniu jest już
//  równa aktualnej liczbie. Te testy pilnują drugiej połowy.
// =============================================================================

test('PO PRZELICZENIU baza przestaje być nieaktualna — próg nie odpala się w kółko', () => {
  // Stan tuż po buildCollectionProjection: projection.chunkCount === liczba fragmentów.
  assert.equal(bazaNieaktualna(560, 560), false);
  assert.equal(bazaNieaktualna(600, 600), false);
});

test('kolejna partia po przeliczeniu NIE przekracza progu', () => {
  // 560 fragmentów w bazie, dochodzi jedna partia (32) — 5,7% to daleko od 30%.
  assert.equal(bazaNieaktualna(592, 560), false);
});

test('dopiero SZEŚCIOKROTNY przyrost, jak w zgłoszeniu, wymaga przeliczenia', () => {
  // Objaw z pola: „układ pochodzi z 96 fragmentów, kolekcja ma 600".
  assert.equal(bazaNieaktualna(600, 96), true);
});
