...
import { createEmbeddingProvider, niezgodnoscModelu, embedConfigFromGlobal } from './embedding.js';
...
// Konfiguracja providera z globalnych ustawień — ten sam kształt co w documents.js.
...
export async function searchCollection(
  ...
  // --- Kontrola spójności modelu PRZED wyszukiwaniem (sekcja 11) ---------------
  // Ta sama ochrona co w embedNextBatch: nigdy nie porównuj wektorów z dwóch modeli.
  ...
  const niezgodnosc = niezgodnoscModelu(coll, config.embed);
  if (niezgodnosc) throw err('model_mismatch', niezgodnosc);
  ...
  const provider = deps.provider || createEmbeddingProvider(embedConfigFromGlobal(config));
