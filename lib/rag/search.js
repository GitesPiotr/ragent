...
export async function searchCollection(
  ...
  const { data: coll, error: cErr } = await client
    ...
    .select('id, embed_model, embed_dim')
    .eq('id', collectionId)
    .single();
  if (cErr || !coll) throw err('not_found', 'Kolekcja nie istnieje.');

  if (coll.embed_model && coll.embed_model !== config.embed.model) {
    throw err(
      'model_mismatch',
      `Kolekcja zbudowana modelem "${coll.embed_model}", a konfiguracja używa "${config.embed.model}". Wyniki byłyby bez sensu — ustaw RAG_EMBED_MODEL na model kolekcji.`
    );
  }
