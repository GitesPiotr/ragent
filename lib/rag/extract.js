...
// Formaty v1 (redline 7): pdf, docx, txt, md, csv. Reszta → invalid_input.
export async function extractBlocks(file, cfg) {
  ...
  if (ext === 'csv') return { blocks: extractCsv(bytesToString(bytes), cfg.size), pageCount: null };
  if (ext === 'md' || ext === 'markdown') return { blocks: extractMd(bytesToString(bytes)), pageCount: null };
  if (ext === 'txt' || ext === 'text') return { blocks: extractTxt(bytesToString(bytes)), pageCount: null };

  throw fail('invalid_input', `Nieobsługiwany format pliku: .${ext}. Obsługiwane: pdf, docx, txt, md, csv.`);
}
