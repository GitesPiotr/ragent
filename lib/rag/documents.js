...
// Odszukanie dokumentu po znaczniku powiązania, W OBRĘBIE KOLEKCJI — bo tak samo
// zakresowany jest unikalny indeks z migracji 018 (collection_id, external_ref).
// Brak → null, nie wyjątek: „tego pliku jeszcze nie zindeksowano" jest zwykłym
// stanem, o który warstwa wyżej pyta przy każdym wyświetleniu listy.
export async function findDocumentByExternalRef(collectionId, externalRef, deps = {}) {
  if (!collectionId || !externalRef) return null;
  const client = deps.client || getSupabaseClient();
...
// Wersja hurtowa findDocumentByExternalRef — JEDNO zapytanie zamiast N.
//
// !!! DZIŚ NIEUŻYWANA — ZOSTAJE ŚWIADOMIE, tak jak external_ref w schemacie !!!
// Powstała, gdy narzędzie agenta mapowało knowledge_file_ids na documentIds.
// Po zmianie modelu agent wskazuje CAŁĄ KOLEKCJĘ (agents.rag_collection_id),
// więc nie ma czego mapować — documentIds jest teraz null.
// Kolumna rag_documents.external_ref (migracja 018) też zostaje nieużywana;
// kasującej migracji świadomie NIE piszemy: kolumna nikomu nie przeszkadza,
// a jej usunięcie byłoby nieodwracalne przy pierwszej zmianie zdania.
//
// Zwraca tablicę zmapowanych dokumentów. Brak dopasowania nie jest błędem.
export async function findDocumentsByExternalRefs(collectionId, externalRefs, deps = {}) {
  const refs = (Array.isArray(externalRefs) ? externalRefs : []).filter(
    (r) => typeof r === 'string' && r
  );
  if (!collectionId || !refs.length) return [];
