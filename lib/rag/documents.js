...
import { refreshProjectionAfterIndexing, projectPendingChunks, kolorDokumentu } from './map.js';
...
export async function getEmbedProgress(documentId, deps = {}) {
  ...
  // =============================================================================
  //  builtAt W POSTĘPIE — ŻEBY MAPA WIEDZIAŁA, KIEDY NAPRAWDĘ WARTO SIĘ ODŚWIEŻYĆ
  //
  //  Odpytywanie mapy decydowało dotąd na podstawie licznika `done`, czyli
  //  KORELATU: nowe wektory zwykle znaczą nowe punkty, ale przeliczenia bazy nie
  //  wykrywają wcale. Zmierzone: przez 111 s indeksowania okno pokazywało układ
  //  policzony z 96 fragmentów, bo `done` rosło, a `builtAt` — to, o co naprawdę
  //  chodzi — nie było widoczne nigdzie w tanim odczycie.
  //
  //  DLACZEGO TUTAJ, A NIE OSOBNYM ŻĄDANIEM: klient i tak woła ten punkt końcowy
  //  co 5 s. `GET /collections/{id}` zwróciłby `builtAt`, ale razem z całą bazą
  //  rzutowania — `mean` i `components` to ponad cztery tysiące liczb, rzędu
  //  100 kB na odczyt. Operator `->>` wyciąga samo pole tekstowe, więc dokładamy
  //  jedno lekkie zapytanie zamiast jednego ciężkiego żądania HTTP.
  //
  //  Brak rzutowania albo błąd odczytu daje `builtAt: null` — mapa traktuje to
  //  jak „bez zmian" i nie odświeża się bez powodu.
  let builtAt = null;
  try {
    ...
    // Postęp indeksowania nie ma prawa paść przez odczyt pomocniczy.
  }

  return { done: total - remaining, total, finished: total > 0 && remaining === 0, builtAt };
}
...
export async function embedNextBatch(documentId, deps = {}) {
  ...
  //
  //  Para (embed_provider, embed_model) z wiersza kolekcji NAPĘDZA dostawcę,
  //  którym liczymy tę partię. RAG_EMBED_PROVIDER i RAG_EMBED_MODEL to
  //  DOMYŚLNE DLA NOWYCH kolekcji (collections.js:85) — nie sterują tą.
  //  Kolekcja chmurowa indeksuje się przez OpenRoutera także wtedy, gdy
  //  serwer stoi na `ollama`, i odwrotnie.
  //
  //  Do rundy 3 było tu odwrotnie: dostawcę budowano z konfiguracji, a para
  //  z kolekcji służyła wyłącznie do ODMOWY, gdy się z nią rozjechała.
  //  Kolekcja pamiętała wybór, którego nie napędzała — pole potwierdzało
  //  zapis bez skutku. Reguła i mapowanie: embedding.js, `embedConfigDlaKolekcji`.
  //
  //  Wiersz kolekcji CZYTAMY MIMO TO przy wstrzykniętym `deps.provider`,
  //  bo strażnik (niżej) ma działać niezależnie od szwu testowego.
  // =========================================================================
  const { data: coll } = await client
    .from('rag_collections')
  ...
  // Strażnik: para, dla której rdzeń nie ma implementacji, ma odmówić czytelnie
  // ZANIM ruszymy status dokumentu — a nie wywrócić się na pierwszym wektorze.
  const nieobslugiwana = nieobslugiwanaPara(coll);
  if (nieobslugiwana) throw err('model_mismatch', `${nieobslugiwana} Indeksowanie wstrzymane.`);
  ...
  try {
    ...
    // LISTA DOKUMENTÓW PRZY NOWYCH PUNKTACH — bez niej punkty są SZARE.
    //
    // Kolor fragmentu nie bierze się z jego `documentId`, tylko z POZYCJI jego
    // dokumentu na liście `documents` (koloryDokumentow → kolorDokumentu(i)).
    // `newChunks` niosą documentId od zawsze, ale mapa dokleja je do stanu, w którym
    // lista dokumentów pochodzi z ostatniego pełnego odczytu — a świeżo wgranego
    // dokumentu tam nie ma. `kolory.get(documentId)` zwracało undefined i punkt
    // dostawał kolor zastępczy. Stąd szare kropki w widoku osadzonym (który żyje
    // z newChunks) i poprawne w oknie (które robi pełny odczyt).
    //
    // Lista jest krótka i budowana tym samym zapytaniem co w getMapData — ta sama
    // kolejność `created_at ascending`, więc te same indeksy, więc te same kolory.
    if (wynik.newChunks) {
      const { data: docRows } = await client
        ...
        .order('created_at', { ascending: true });
      if (docRows) {
        wynik.documents = docRows.map((d, i) => ({ id: d.id, name: d.file_name, color: kolorDokumentu(i) }));
      }
    }
  } catch (e) {
    // INDEKSOWANIE IDZIE DALEJ — ta decyzja zostaje: mapa jest warstwą wizualizacji,
    // a wektory są policzone poprawnie i nie wolno ich stracić przez awarię rzutowania.
    //
    // ALE BŁĄD PRZESTAJE ZNIKAĆ. Wcześniej `catch {}` połykał wszystko, więc nieudane
    // przeliczenie bazy było nierozróżnialne od udanego: dokument dostawał `ready`,
    // mapa zostawała ze starym rzutem i nikt się nie dowiadywał dlaczego. Teraz powód
    // wraca w odpowiedzi i klient może go pokazać zamiast samego „przelicz mapę".
    wynik.mapaBlad = e && e.message ? e.message : 'nieznany błąd rzutowania';
  }
