...
export async function ingestFile({ collectionId, file }, deps = {}) {
  ...
  try {
    ...
    if (up.error) {
      ...
      if (/row-level security|not authorized|permission|violates/i.test(msg)) {
        throw err(
          'internal',
          `Brak uprawnień do zapisu w Storage (RLS). Sprawdź, czy RAG_SUPABASE_SERVICE_KEY to klucz service_role/secret, a nie publishable/anon. Szczegóły: ${msg}`
        );
      }
...
export async function listDocumentChunks(documentId, { limit = 300 } = {}) {
...
export async function reindexDocument(documentId, { rechunk = false } = {}) {
  if (!documentId) throw err('invalid_input', 'Brak identyfikatora dokumentu.');
  const config = getConfig();
  const client = getSupabaseClient();
