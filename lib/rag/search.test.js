...
// ODWRÓCONE W RUNDZIE 3 — patrz bliźniaczy test w documents.test.js. Kolekcja
// na innym modelu niż konfiguracja to nie awaria, tylko normalna sytuacja:
// wektor zapytania powstaje parą Z KOLEKCJI.
test('kolekcja na innym modelu niż konfiguracja przeszukuje się NORMALNIE', async () => {
  await withEnv({ RAG_EMBED_MODEL: 'bge-m3' }, async () => {
    const spy = {};
    const provider = makeProvider(spy);
    const client = makeFakeClient({
      collection: { id: 'k1', embed_provider: 'openrouter', embed_model: 'baai/bge-m3', embed_dim: 1024 },
      rows: [row('c1', 0.2)],
    });
    const r = await searchCollection({ collectionId: 'k1', query: 'urlop' }, { client, provider });
    assert.equal(r.hits.length, 1);
    assert.equal(spy.queryCalls, 1);
  });
});

test('model_mismatch: dostawca kolekcji bez implementacji, provider nie jest wołany', async () => {
  const spy = {};
  const provider = makeProvider(spy);
  const client = makeFakeClient({
    collection: { id: 'k1', embed_provider: 'voyage', embed_model: 'voyage-3', embed_dim: 1024 },
  });

  await assert.rejects(
    () => searchCollection({ collectionId: 'k1', query: 'urlop' }, { client, provider }),
    (e) => e.code === 'model_mismatch' && /voyage/.test(e.message)
  );
  assert.equal(spy.queryCalls, 0, 'nie licz wektora, skoro nie ma czym go policzyć');
});
