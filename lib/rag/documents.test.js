...
function makeState(nChunks, embedModel = 'bge-m3', docStatus = 'chunked') {
  const chunks = [];
  ...
  return {
    ...
    rag_chunks: chunks,
    rag_collections: [{ id: 'k1', embed_model: embedModel }],
  };
}
...
test('embedNextBatch liczy JEDNĄ partię i raportuje postęp', async () => {
  await withBatch(2, async () => {
    ...
    assert.deepEqual(r, { done: 2, total: 5, finished: false });
...
test('wznawialność: kolejne wywołania kończą dokument bez dublowania', async () => {
  await withBatch(2, async () => {
    ...
    assert.deepEqual(r1, { done: 2, total: 5, finished: false });
    assert.deepEqual(r2, { done: 4, total: 5, finished: false });
    assert.deepEqual(r3, { done: 5, total: 5, finished: true });
...
test('wywołanie po zakończeniu → finished, done=total, status ready', async () => {
  await withBatch(10, async () => {
    ...
    assert.deepEqual(r, { done: 3, total: 3, finished: true });
    assert.equal(state.rag_documents[0].status, 'ready');
  });
});

test('model_mismatch: kolekcja na innym modelu niż konfiguracja', async () => {
  await withBatch(2, async () => {
...
test('getEmbedProgress po przerwaniu zwraca stan z bazy, nie zero', async () => {
  await withBatch(2, async () => {
    ...
    assert.deepEqual(p, { done: 4, total: 7, finished: false });
  });
});
...
test('getEmbedProgress: dokument w całości policzony → finished', async () => {
  await withBatch(10, async () => {
    ...
    assert.deepEqual(await getEmbedProgress('d1', { client: deps.client }), { done: 3, total: 3, finished: true });
  });
});
