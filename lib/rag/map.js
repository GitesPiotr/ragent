...
export async function buildCollectionProjection(collectionId, deps = {}) {
  ...
  const { data: coll, error: cErr } = await client
    ...
    .select('id, embed_model, projection, updated_at')
    .eq('id', collectionId)
    .single();
  if (cErr || !coll) throw err('not_found', 'Kolekcja nie istnieje.');

  // ZNACZNIK DO STRAŻNIKA — czytany PRZED liczeniem, sprawdzany przy zapisie.
  const updatedAtNaStarcie = coll.updated_at;
  ...
  // =============================================================================
  //  STRAŻNIK: ZAPIS WARUNKOWY, NIE BLOKADA W PAMIĘCI
  //
  //  Odkąd rzutowanie buduje TAKŻE okno (a nie tylko widok prowadzący indeksowanie),
  //  dwa widoki mogą wejść tu jednocześnie. Blokada w pamięci procesu tego nie
  //  załatwi: docs/rag-SPEC.md:81 zakłada hosting typu Vercel, gdzie dwa żądania
  //  trafiają na dwie różne instancje, każdą z własną, pustą mapą blokad.
  //
  //  Dlatego strażnik siedzi w BAZIE i nie wymaga migracji: zapisujemy tylko wtedy,
  //  gdy `updated_at` jest nadal ten, który widzieliśmy przed liczeniem. Postgres
  //  rozstrzyga to atomowo. Kto przegra, NIE nadpisuje cudzego rzutowania — bo
  //  rozjazd `projection` z zapisanymi współrzędnymi to mapa rysująca punkty
  //  w układzie, którego jej własna baza nie opisuje.
  //
  //  CZEGO TEN STRAŻNIK NIE ROBI, żeby nie było złudzeń: nie zapobiega podwójnemu
  //  LICZENIU (oba przebiegi wykonają PCA) ani nie cofa `saveCoords` przegranego.
  //  Pełna wzajemność wymagałaby transakcji, a przez PostgREST jej nie mamy.
  //  Chroni to, co najgroźniejsze — niespójność między `projection` a coord_x/y/z.
  const { error: uErr, count } = await client
    .from('rag_collections')
    .update({ projection, updated_at: new Date().toISOString() }, { count: 'exact' })
    .eq('id', collectionId)
    .eq('updated_at', updatedAtNaStarcie);
  if (uErr) throwDb(uErr);
  const tZapis = Date.now() - t3;

  if (count === 0) {
    // Ktoś zbudował w międzyczasie. To nie jest błąd — mapa JEST zbudowana,
    // tylko nie naszym przebiegiem. Wołający ma to odróżnić od własnego sukcesu.
    return { built: false, ubiegnietoNas: true, chunkCount: vectors.length };
  }
