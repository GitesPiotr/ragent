...
export async function createCollection(input, deps = {}) {
  ...
  // DOSTAWCA ZAPISYWANY RAZEM Z MODELEM (migracja 021). Bierzemy go z tej samej
  // konfiguracji, którą właśnie odpytała sonda — inaczej dałoby się zapisać
  // parę, której nikt nie sprawdził. Straznik zgodności porównuje potem OBIE
  // wartości; uzasadnienie pary w nagłówku 021_embed_provider.sql.
  const embedProvider = config.embed.provider || 'ollama';
