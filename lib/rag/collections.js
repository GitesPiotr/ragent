...
// Zwraca kolekcję o danym `external_ref`, tworząc ją, jeśli jeszcze nie istnieje.
//
// !!! DZIŚ NIEUŻYWANA — ZOSTAJE ŚWIADOMIE !!!
// Powstała dla modelu „konto → jedna kolekcja zakładana automatycznie", który
// został wycofany: Baza wiedzy i Kreator RAG to teraz dwa osobne narzędzia,
// a agent WSKAZUJE kolekcję (agents.rag_collection_id, migracja 019) zamiast
// dostawać ją z automatu. Nic jej nie woła.
// Nie kasuję jej, bo jest poprawna i tania — hak `external_ref` w rag_collections
// istnieje od Sesji 2 właśnie po to, żeby dało się wiązać kolekcje z czymkolwiek
// z zewnątrz. Gdyby wrócił pomysł kolekcji zakładanych automatycznie (per zespół,
// per projekt), to jest gotowa i przetestowana wyścigiem implementacja.
//
// GRANICA Z SEKCJI 3 JEST NIENARUSZONA: rdzeń nie wie, czym jest `ref`. Dostaje
// TEKST i traktuje go jak nieprzezroczysty znacznik. To warstwa wyżej decyduje,
// że tym tekstem jest identyfikator konta — tutaj równie dobrze mógłby stać numer
// sprawy albo identyfikator zespołu, bez zmiany ani jednej linii.
//
// WYŚCIG JEST OBSŁUŻONY, BO JEST REALNY: dwa równoczesne żądania indeksowania
// (dwa pliki, dwa kliknięcia) zastają brak kolekcji i oba ruszają ją tworzyć.
// Unikalny indeks częściowy na external_ref (session-2-schema.sql:35) przepuszcza
// pierwsze, a drugiemu zwraca 23505. Nie jest to błąd do pokazania użytkownikowi —
// znaczy dokładnie tyle, że ktoś nas ubiegł, a my chcemy tego samego wyniku.
// Dlatego na 23505 pobieramy jeszcze raz, zamiast rzucać.
//
// DLACZEGO NIE `upsert`: createCollection SONDUJE MODEL po wymiar (probeModelDim),
// czyli wykonuje pracę, zanim dojdzie do zapisu. Upsert wykonywałby ją zawsze,
// także wtedy, gdy kolekcja od dawna istnieje — a to wywołanie do Ollamy przy
// każdym indeksowaniu pliku.
export async function ensureCollectionByExternalRef(ref, opcje = {}, deps = {}) {
  const znacznik = String(ref || '').trim();
  if (!znacznik) throw err('invalid_input', 'Brak znacznika powiązania (external_ref).');

  const istniejaca = await getCollectionByExternalRef(znacznik, deps);
  if (istniejaca) return istniejaca;

  try {
    return await createCollection(
      {
        name: opcje.name || znacznik,
        description: opcje.description || null,
        externalRef: znacznik,
        embedModel: opcje.embedModel,
      },
      deps
    );
  } catch (e) {
    // 23505 przychodzi z createCollection już przetłumaczone na invalid_input
    // z komunikatem o istniejącym powiązaniu — stąd rozpoznanie po treści,
    // nie po kodzie bazy. Patrz createCollection powyżej.
    if (e && e.code === 'invalid_input' && /external_ref/i.test(e.message || '')) {
      const poWyscigu = await getCollectionByExternalRef(znacznik, deps);
      if (poWyscigu) return poWyscigu;
    }
    throw e;
  }
}
