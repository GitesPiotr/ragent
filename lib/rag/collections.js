...
export function normalizeCreateInput(input, defaultModel) {
  ...
  const model =
    typeof input.embedModel === 'string' && input.embedModel.trim()
      ...
      : defaultModel;
  // DOSTAWCA razem z modelem (runda 2). Brak = dziedziczy z konfiguracji,
  // dokładnie jak model — warstwa HTTP i tak przepuszcza tu wyłącznie pary
  // z oferty (lib/config/modeleEmbeddingow.js), a rdzeń nie zna tej oferty
  // i znać nie ma.
  const provider =
    typeof input.embedProvider === 'string' && input.embedProvider.trim()
      ? input.embedProvider.trim()
      : null;

  return { name, description, externalRef, embedModel: model, embedProvider: provider };
}
...
export async function createCollection(input, deps = {}) {
  ...
  // DOSTAWCA ZAPISYWANY RAZEM Z MODELEM (migracja 021). Wybór z żądania ma
  // pierwszeństwo przed konfiguracją serwera — od rundy 2 to użytkownik
  // decyduje przy tworzeniu kolekcji, a RAG_EMBED_PROVIDER zostaje wyłącznie
  // domyślką dla wywołań, które wyboru nie podają.
  const embedProvider = norm.embedProvider || config.embed.provider || 'ollama';

  // Sonda wymiaru (sekcja 9) idzie TĄ SAMĄ PARĄ, która zaraz trafi do bazy —
  // nie konfiguracją serwera. Inaczej dałoby się zapisać kolekcję na dostawcy,
  // którego nikt nie odpytał, z wymiarem zmierzonym gdzie indziej.
  const dim = await probeModelDim(norm.embedModel, {
    ...config.embed,
