...
export const ragSearch = {
  ...
  async execute({ pytanie }, ctx = {}) {
    ...
    if (!idKolekcji) {
      return powiedz(
        "Ten agent nie ma wskazanej kolekcji dokumentów, więc nie ma czego przeszukać. " +
          "Powiedz użytkownikowi, że nie masz przypisanej kolekcji, i zaproponuj, " +
          "żeby wybrał ją w kreatorze agenta, w karcie „RAG” — przy przełączniku " +
          "„Przeszukiwanie dokumentów” jest lista „Przeszukiwana kolekcja”.",
      );
    }
    ...
    try {
      ...
      // Klucz dostawcy chmurowego embeddingów (od rundy 1 cyklu RAG-embed).
      // Osobna gałąź, bo to sytuacja do NAPRAWIENIA W KONFIGURACJI, a nie
      // powód, żeby model cokolwiek proponował użytkownikowi.
      if (kod === "no_embed_key") {
        return powiedz(
          "Wyszukiwanie w dokumentach nie działa, bo brakuje klucza dostawcy embeddingów " +
            `(${e.message}). Powiedz to użytkownikowi i odpowiedz bez dokumentów.`,
        );
      }
      if (kod === "model_mismatch" || kod === "dim_mismatch") {
        // =================================================================
        //  GAŁĄŹ ŻYWA, ALE ZNACZY CO INNEGO NIŻ DO RUNDY 3.
        //
        //  Rozstrzygnięcie sprawdzone przy zmianie źródła prawdy: oba kody
        //  DALEJ POTRAFIĄ TU DOLECIEĆ, tylko z innych powodów.
        //
        //   • `model_mismatch` NIE ZNACZY JUŻ „kolekcja rozjechała się
        //     z konfiguracją serwera". Ta sytuacja przestała być błędem —
        //     kolekcja napędza własnego dostawcę (search.js). Kod niesie
        //     teraz JEDYNY przypadek, który został: kolekcja zbudowana
        //     dostawcą, dla którego ta wersja nie ma implementacji.
        //   • `dim_mismatch` jest nietknięty: wciąż wychodzi z bazy, gdy
        //     wymiar wektora zapytania nie pasuje do kolumny (search.js,
        //     throwRpc). To awaria schematu, nie wyboru użytkownika.
        //
        //  DLATEGO ZMIENIŁO SIĘ ZDANIE DLA MODELU. Poprzednie kazało mu
        //  wskazać „które dwie wartości się nie zgadzają" — przy nowym
        //  znaczeniu `model_mismatch` nie ma dwóch stron do porównania,
        //  jest jedna kolekcja nie do obsłużenia. Komunikat rdzenia w obu
        //  przypadkach mówi wprost, czego brakuje, więc podajemy go bez
        //  skracania i nie każemy modelowi zgadywać naprawy: żadnej z tych
        //  dwóch użytkownik nie naprawi w czacie.
        // =================================================================
        return powiedz(
          "Wyszukiwania w dokumentach nie da się wykonać dla tej kolekcji " +
            `(${e.message}). Powtórz użytkownikowi ten powód dosłownie, nie proponuj obejść ` +
            "i odpowiedz bez dokumentów.",
        );
      }
      return powiedz(
