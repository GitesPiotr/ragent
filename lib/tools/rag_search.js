  ...
  async execute({ pytanie }, ctx = {}) {
    ...
    try {
      ...
      if (kod === "model_mismatch" || kod === "dim_mismatch") {
        // Komunikat z rdzenia niesie od migracji 021 OBIE wartości —
        // dostawcę i model, po jednej stronie i po drugiej — więc podajemy go
        // dalej bez skracania. „Wymagają przeindeksowania" ZNIKŁO stąd
        // celowo: przy niezgodnej PARZE naprawą jest zwykle poprawienie
        // konfiguracji (ten sam plik wag u innego dostawcy), a nie liczenie
        // korpusu od nowa. Odsyłanie do przeindeksowania kazałoby użytkownikowi
        // wykonać kilkanaście minut pracy zamiast zmienić jedną zmienną.
        return powiedz(
          "Wyszukiwanie w dokumentach jest niespójne z modelem, którym je zindeksowano " +
            `(${e.message}). Powiedz użytkownikowi dokładnie, które dwie wartości się nie zgadzają.`,
        );
      }
      return powiedz(
