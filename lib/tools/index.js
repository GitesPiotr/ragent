...
// Format narzedzia dla Ollamy (zblizony do OpenAI).
function toOllamaDef({ name, description, input_schema }) {
  return {
    type: "function",
...
export function getToolDefsForOllama(enabledIds = []) {
  const defs = enabledLocalTools(enabledIds, "ollama").map(toOllamaDef);
  if (enabledIds.includes("web_search")) {
    defs.push(toOllamaDef(webSearchLocal));
  }
  return defs;
}
...
// ctx — opcjonalny kontekst (np. { sources: [] }); narzedzia moga tam odlozyc
// dodatkowe dane dla UI. Narzedzia, ktore go nie potrzebuja, ignoruja go.
