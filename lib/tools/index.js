...
export const TOOLS = {
  ...
  // Przeszukiwanie dokumentow agenta. BEZ requiresProvider i BEZ providerSide —
  // dziala u kazdego dostawcy, bo cala praca dzieje sie u nas (rdzen lib/rag/),
  // a model tylko zadaje pytanie i dostaje tekst.
  //
  // TO JEST PRZELACZNIK TRYBU WIEDZY. Agent z wlaczonym rag_search pyta
  // o fragmenty; bez niego dziala stara sciezka doklejania calych plikow
  // do promptu (buildKnowledgeBlock, limit 24 000 znakow). Oba MOGA dzialac
  // naraz i to jest swiadomy tryb 3 ze SPEC 18 — zadnego if-a wykluczajacego.
  // Zero nowych kolumn: przelacznikiem jest agents.tools, ktore juz istnieje.
  [ragSearch.id]: ragSearch,
};
...
// To samo dla OpenRoutera. Osobna funkcja, choc dzis zwraca DOKLADNIE to samo
// co powyzsza — bo filtr `requiresProvider` porownuje z przekazana nazwa
// dostawcy. Wolanie tu getToolDefsForOpenAI dzialaloby przez przypadek: oba
// narzedzia wymagajace dostawcy celuja w "anthropic" i "ollama", wiec zaden
// z nich nie przechodzi ani dla "openai", ani dla "openrouter". Pierwsze
// narzedzie z requiresProvider: "openai" po cichu wlaczyloby sie OpenRouterowi.
export function getToolDefsForOpenRouter(enabledIds = []) {
  return enabledLocalTools(enabledIds, "openrouter").map(toFunctionDef);
}
