...
export function toReadableError(error, context) {
  ...
  // Brak uprawnien / RLS blokuje odczyt.
  if (code === "42501" || /permission denied|row-level security/i.test(msg)) {
    return "Baza odrzuciła zapytanie z powodu uprawnień (RLS). Sprawdź, czy w Supabase tabele projects i agents nie mają włączonego Row Level Security.";
  }
  // Zly klucz API.
