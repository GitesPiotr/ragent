...
// Identyfikator albo NULL. Pusty string z <select> MUSI stac sie nullem —
// kolumna jest typu uuid i "" odrzuci zapis bledem skladni, ktory dla
// uzytkownika wyglada jak awaria zapisu calego agenta.
...
export function dbAgentToState(row) {
  ...
  return {
    ...
    // NULL = agent nie korzysta z RAG i to jest stan domyslny. Pusty string
    // z <select> tez ma znaczyc NULL — stad normalizacja przez toIdOrNull,
    // inaczej do bazy poszedlby "" i uuid odrzucilby zapis.
    rag_collection_id: toIdOrNull(row.rag_collection_id),
...
export function stateAgentToDbPatch(agent) {
  ...
  return {
    ...
    rag_collection_id: toIdOrNull(a.rag_collection_id),
  };
}
...
export function isSameConfig(a, b) {
  ...
  return (
    ...
    toIdOrNull(a.rag_collection_id) === toIdOrNull(b.rag_collection_id)
  );
}
