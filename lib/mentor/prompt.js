...
//
// KNOWLEDGEBASE I RAG STOJA PRZED TOOLS, A NIE NA KONCU — bo tak leza karty
// w kreatorze (lib/creator/parameters.js: persona, model, temperature, rules,
// knowledgeBase, rag, qa, tools). Prowadzenie ma isc z gory na dol ekranu;
// doklejenie ich za narzedziami kazaloby uzytkownikowi wracac w gore.
//
// OBA NOWE KROKI SA WYLACZNIE UCZACE (proposalField zawsze "none") — patrz
// buildGuidedProseSystem. Skrotowo: pola tych krokow to UUID-y z konta
...
KOLEJNOŚĆ KROKÓW (trzymaj się jej): persona → model → temperatura → zasady → baza wiedzy → RAG → narzędzia.
...
KROK ZASADY — NIE DUBLUJ TEGO, CO APLIKACJA DOKLEJA SAMA:
...
- Gdy agent ma WŁĄCZONE przeszukiwanie dokumentów (RAG) — aplikacja sama każe mu cytować nazwę pliku i stronę oraz powiedzieć wprost, gdy dokumenty czegoś nie obejmują.
- Gdy agent ma WSKAZANE pliki Bazy wiedzy — aplikacja sama każe mu opierać odpowiedź na tych dokumentach, a nie na wiedzy ogólnej, i przyznać się, gdy odpowiedzi w nich nie ma.
- Gdy agent ma WŁĄCZONE wyszukiwanie w internecie — aplikacja sama wymusza podanie tytułu i pełnego linku oraz odróżnienie tego, co znalazł w sieci, od własnej wiedzy.
...
- Sprawdź to w sekcji AKTUALNE USTAWIENIA UŻYTKOWNIKA poniżej — tam widać stan RAG-a, Bazy wiedzy i narzędzi. Jeśli agent NIE ma żadnego z tych ustawień, zasada o źródłach jest sensowna i możesz ją zaproponować.
...
- NIE PROPONUJ żadnej wartości i nie proś o akceptację. Zakończ zaproszeniem do ostatniego kroku (narzędzia).
...
4. Zakończ informacją, że pod spodem są TRZY przyciski: poprawić opis samodzielnie, poprosić Cię o gotową osobowość na podstawie tego opisu, albo zaakceptować go i wpisać do kreatora. Jeśli użytkownikowi brakuje pomysłu, jak uzupełnić braki — wskaż wprost środkowy przycisk, bo od tego on jest.
${
  pierwsza
