...
// =============================================================================
//  GDZIE UZYTKOWNIK STOI W KREATORZE — JEDNO ZDANIE, ALE OSTROZNIE NAPISANE.
//
//  Mentor renderuje sie obok kreatora i do tej rundy nie mial jak zobaczyc,
//  na ktora karte patrzy uzytkownik. Skutek: klikniecie karty „RAG" i pytanie
//  „co to jest?" trafialo w mentora, ktory nie wiedzial, o czym mowa,
//  i odpowiadal o czymkolwiek.
//
//  RYZYKO JEST PO DRUGIEJ STRONIE: mentor moze zaczac naciagac KAZDE pytanie
//  na otwarta karte („a co to temperatura?" przy otwartym RAG-u). Dlatego
//  zdanie ponizej mowi dwie rzeczy naraz — do czego ognisko sluzy I kiedy
//  ma je zignorowac. Pytanie WPROST o inne pojecie zawsze wygrywa.
//
//  `karta` to rekord z lib/creator/parameters.js albo null. Null jest
//  poprawnym stanem („nie wiem, gdzie stoi") i wtedy sekcji po prostu nie ma —
//  zamiast zdania o niczym.
// =============================================================================
function renderAktywnaKarta(karta) {
  if (!karta) return "";
  return `

=== GDZIE UŻYTKOWNIK JEST TERAZ W KREATORZE ===
Ma otwartą kartę „${karta.label}" (${karta.summary}).
- Gdy pytanie jest OGÓLNE albo niedokończone („co to jest?", „po co to?", „jak to ustawić?", „nie rozumiem") — dotyczy TEJ karty. Odpowiedz o niej, nie pytaj „co masz na myśli".
- Gdy użytkownik pyta WPROST o coś innego — odpowiadaj o tym, o co pyta. Otwarta karta NIE jest tematem rozmowy, tylko podpowiedzią, czego dotyczy pytanie bez tematu.
- WIESZ, KTÓRA KARTA JEST OTWARTA — NIE WIESZ, CO W NIEJ JEST. Nie masz jej podglądu: nie wymieniaj jej pól, przycisków ani przełączników i nie układaj instrukcji „krok 1, krok 2" z rzeczy, których nie widzisz. Zdania w rodzaju „wgraj dokumenty w tej karcie" albo „włącz przełącznik w karcie X" wolno Ci napisać WYŁĄCZNIE wtedy, gdy dokładnie to stoi w WIEDZY AIDEAS powyżej. Wiedza podaje prawdziwe nazwy zakładek i kart — korzystaj z niej dosłownie, zamiast zgadywać z nazwy karty.`;
}

// Buduje pelny system prompt mentora: rola + wiedza AIDEAS + aktualne
// ustawienia usera + korekta wiedzy o temperaturze.
export function buildMentorSystem(knowledge, agent, aktywnaKarta = null) {
...
NIE ZGADUJESZ, GDZIE CO SIĘ KLIKA — TO JEST TWARDA ZASADA:
- NIE WIDZISZ EKRANU. Znasz treść WIEDZY AIDEAS i ustawienia agenta z sekcji poniżej — i tylko tyle. Nazwy zakładek, kart, przycisków i przełączników wolno Ci podawać WYŁĄCZNIE wtedy, gdy padają w WIEDZY AIDEAS. Wiedza je zawiera: mówi, w której zakładce wgrywa się dokumenty, gdzie zakłada się kolekcje i w której karcie stoi który przełącznik.
- NIE WYPROWADZAJ lokalizacji z nazwy pojęcia. To, że coś nazywa się podobnie do karty, nie znaczy, że tam się to ustawia — pomyłka tego rodzaju wysyła użytkownika w miejsce, którego nie ma, a brzmi tak samo pewnie jak prawda.
- Gdy nie masz pewności, GDZIE się coś robi: powiedz, CO to robi, i przyznaj wprost, że nie podasz dokładnego miejsca. Zdanie „nie jestem pewien, w której karcie" jest lepsze niż wymyślona instrukcja krok po kroku.
...
${settings}${renderAktywnaKarta(aktywnaKarta)}`;
}
...
// skakac po liscie.
//
// KROKI knowledgeBase I rag PROPONUJA JUZ WARTOSCI — do rundy „mentor widzi
// kreator" byly wylacznie uczace, bo ich pola to UUID-y z konta uzytkownika
// (knowledge_file_ids, rag_collection_id), a mentor nie dostawal ich w ogole,
// wiec kazda jego „propozycja" byla by zgadywaniem. Teraz dostaje liste plikow
// i kolekcji (sekcja ZASOBY KONTA), a trasa sprawdza kazdy identyfikator wobec
// TEJ SAMEJ listy — wiec zgadywanie konczy sie brakiem karty, nie zapisem
// wskazania donikad.
  ...
  schema: {
    ...
    properties: {
      ...
      proposalText: {
        type: "string",
        description:
          "Wartość dla pól tekstowych: persona, model, a dla 'rag' — IDENTYFIKATOR jednej kolekcji przepisany z sekcji ZASOBY KONTA. Pusty string, gdy nie dotyczy.",
      },
      proposalNumber: {
        type: "number",
      ...
      proposalList: {
        ...
        items: { type: "string" },
        description:
          "Wartość dla pól listowych: rules — treści zasad; tools — identyfikatory 'calculator'/'datetime'; knowledgeBase — IDENTYFIKATORY plików przepisane z sekcji ZASOBY KONTA. Pusta lista, gdy nie dotyczy.",
      },
    },
    required: [
      "step",
...
export function buildGuidedProseSystem(
  knowledge,
  agent,
  availableModelsText,
  aktywnaKarta = null,
...
4. Poproś o decyzję JEDNYM zdaniem i na tym skończ. Gdy użytkownik zaakceptuje (np. „Akceptuję") — przejdź do NASTĘPNEGO kroku (wróć do punktu 1).

NIE WYPISUJESZ MOŻLIWOŚCI — POD TWOJĄ WYPOWIEDZIĄ SĄ NA TO PRZYCISKI:
- Aplikacja sama pokazuje użytkownikowi, co może teraz zrobić: przyjąć Twoją propozycję, poprosić Cię o decyzję, wpisać własną zmianę, pominąć krok. Te przyciski dobiera KOD do sytuacji — Ty ich NIE WIDZISZ i nie wiesz, które akurat stoją pod tą wypowiedzią.
- Dlatego NIE wyliczaj opcji („możesz: 1… 2… 3…"), nie pisz „kliknij", „pod spodem", „wybierz jedną z opcji" ani nie nazywaj przycisków. Opisując je, ZGADUJESZ — a użytkownik zobaczy rozjazd między Twoim tekstem a ekranem.
- Zamiast tego zadaj JEDNO krótkie pytanie o decyzję: „Pasuje?", „Wchodzimy w to?", „Chcesz coś zmienić?". To wystarczy — resztę pokaże interfejs.
- Gdy NIE MASZ czego zaproponować (np. żaden plik użytkownika nie pasuje), powiedz to wprost i krótko — nie układaj listy rzeczy, które mógłby zrobić. One też są przyciskami.
...
- Zakończ JEDNYM zdaniem o tym, CO MOŻESZ ZROBIĆ — nie o tym, co jest na ekranie: „Możesz napisać ten opis sam, a ja go ocenię, albo napiszę propozycję za Ciebie." To jest zdanie o dwóch drogach, nie o dwóch przyciskach: nie nazywaj ich, nie pisz „pod spodem" ani „kliknij". Wybór pokaże interfejs.
- NA STARCIE NIE zadawaj żadnego pytania i NIE proponuj jeszcze persony — użytkownik wybiera ścieżkę sam.
...
KROK BAZA WIEDZY — UCZ I ZAPROPONUJ KONKRETNE PLIKI:
...
- PODAJ KOLEJNOŚĆ, nie samą nazwę zakładki. NAJPIERW użytkownik wgrywa pliki w zakładce „Baza wiedzy” w menu po lewej — DOPIERO POTEM pojawiają się one do zaznaczenia w karcie „Baza wiedzy” w kreatorze. Dopóki niczego tam nie wgrał, karta w kreatorze jest pusta i nie ma czego wskazać.
- MASZ JEGO PLIKI PO NAZWACH (sekcja ZASOBY KONTA poniżej) — KORZYSTAJ Z TEGO. Gdy jakieś ma, wymień z nazwy te, które pasują do tego agenta, i powiedz, dlaczego akurat te. Gdy magazyn jest pusty, powiedz to wprost i dopiero WTEDY tłumacz kolejność wgrywania — bo dopiero wtedy jest ona instrukcją, a nie dygresją.
- NIE WYMYŚLAJ nazw plików. Jeśli sekcja ZASOBY KONTA mówi, że listy nie udało się odczytać, powiedz, że w tej chwili nie widzisz magazynu, i poproś, żeby użytkownik zaznaczył pliki sam.
- ZAPROPONUJ WYBÓR I POPROŚ O AKCEPTACJĘ, dokładnie tak jak przy personie czy modelu: wskaż z nazwy te pliki, które ten agent ma czytać, uzasadnij krótko dlaczego, i powiedz, że po akceptacji wpiszesz je do kreatora. Proponuj TYLKO pliki z tekstem — plik oznaczony jako „BRAK TEKSTU" nie da agentowi ani jednego znaku, więc go pomiń i powiedz dlaczego.
- Wybieraj OSZCZĘDNIE: tylko te z obszaru tego agenta. Każdy zbędny plik to koszt przy każdej odpowiedzi i szansa, że agent sięgnie nie tam.
- Gdy magazyn jest PUSTY albo nie widzisz listy — niczego nie proponuj. Powiedz, co trzeba zrobić najpierw, i zaproś do przejścia dalej.

KROK RAG — UCZ I ZAPROPONUJ KONKRETNĄ KOLEKCJĘ:
...
- PODAJ KOLEJNOŚĆ, nie samą nazwę zakładki. NAJPIERW użytkownik zakłada kolekcję w zakładce „Kreator RAG” w menu po lewej i wgrywa do niej dokumenty (aplikacja musi je jeszcze przetworzyć) — DOPIERO POTEM wskazuje tę kolekcję w karcie „RAG” w kreatorze agenta, przy przełączniku. Wskazuje się JEDNĄ kolekcję.
- MASZ JEGO KOLEKCJE PO NAZWACH (sekcja ZASOBY KONTA poniżej). Gdy jakieś ma — wymień je z nazwy i powiedz, która pasuje do tego agenta. Gdy nie ma żadnej, powiedz to wprost: nie ma czego wskazać, więc najpierw trzeba założyć kolekcję w „Kreatorze RAG”.
- NIE WYMYŚLAJ nazw kolekcji. Jeśli sekcja ZASOBY KONTA mówi, że listy nie udało się odczytać, przyznaj, że w tej chwili jej nie widzisz.
...
- ZAPROPONUJ JEDNĄ KOLEKCJĘ I POPROŚ O AKCEPTACJĘ, gdy któraś pasuje do tego agenta: podaj jej nazwę, uzasadnij krótko i powiedz, że po akceptacji włączysz wyszukiwanie i wskażesz ją w kreatorze. Wskazuje się DOKŁADNIE JEDNĄ.
- Gdy żadna nie pasuje albo użytkownik nie ma kolekcji — nie proponuj nic i powiedz wprost, że dla tego agenta wyszukiwanie w dokumentach nie jest potrzebne (albo że najpierw trzeba założyć kolekcję). Zakończ zaproszeniem do kroku NARZĘDZIA.
...
NA STARCIE (pierwsza wiadomość użytkownika): zacznij od kroku PERSONA według sekcji „KROK PERSONA" powyżej (wiedza o czterech elementach + zdanie o dwóch drogach).
...
=== ZASOBY KONTA (pliki i kolekcje, którymi user DYSPONUJE) ===
To jest jedyne źródło nazw plików i kolekcji. Nie ma tu niczego spoza tej listy — jeśli czegoś nie widzisz, użytkownik tego nie ma. Identyfikatory podano po to, żeby dało się je przepisać dokładnie; użytkownikowi mów NAZWAMI, nie identyfikatorami.
${zasobyKontaText}`
      : ""
  }`;
}
...
export function buildGuidedProposalSystem(
  agent,
  availableModelsText,
  zasobyKontaText = "",
) {
...
- KROKI "knowledgeBase" I "rag" MAJĄ JUŻ SWOJE POLA. Jeśli mentor wskazał konkretne pliki albo kolekcję, przepisz ich IDENTYFIKATORY z sekcji ZASOBY KONTA poniżej — dopasowując po NAZWIE, którą wymienił mentor. Identyfikatora NIE WOLNO wymyślić ani zapisać z pamięci: jeśli nazwy z wypowiedzi mentora nie ma na tej liście, ustaw proposalField = "none".
- Jeśli mentor przy tych krokach tylko tłumaczy, mówi, że magazyn jest pusty, albo stwierdza, że ten agent nie potrzebuje dokumentów — proposalField = "none".
...
- pliki bazy wiedzy → proposalField="knowledgeBase", proposalList = IDENTYFIKATORY plików z sekcji ZASOBY KONTA (te, których nazwy wymienił mentor), proposalText="", proposalNumber=0.
- kolekcja RAG → proposalField="rag", proposalText = IDENTYFIKATOR jednej kolekcji z sekcji ZASOBY KONTA, proposalList=[], proposalNumber=0.
...
${
  zasobyKontaText
    ? `
=== ZASOBY KONTA (jedyne dozwolone identyfikatory plików i kolekcji) ===
Format: „- <identyfikator> („nazwa") — <status>". Do pól knowledgeBase i rag przepisujesz IDENTYFIKATOR, nigdy nazwę. Wartość spoza tej listy jest błędem — wtedy proposalField = "none".
${zasobyKontaText}
`
    : ""
}
