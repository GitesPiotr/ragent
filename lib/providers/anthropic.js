...
export async function sendChatAnthropic({
  ...
  onEvent,
  responseFormat,
  // Kontekst narzedzi { user, agent, sources } — sklada go app/api/chat/route.js.
  // Domyslka dla wolan bez narzedzi (mentor), zeby ctx.sources zawsze istnialo.
  ctx = { sources: [] },
}) {
  ...
  for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
    ...
    for (const block of toolUseBlocks) {
      ...
      // ctx jako TRZECI ARGUMENT — do rundy 6 go tu nie bylo, przez co narzedzia
      // uruchamiane u domyslnego dostawcy nie mialy ani tozsamosci, ani kanalu
      // `sources`. Ollama miala jedno i drugie, wiec ta sama funkcja zachowywala
      // sie inaczej zaleznie od dostawcy — po cichu, bo brak ctx nie jest bledem.
  ...
  // ZRODLA — DWA NIEZALEZNE POCHODZENIA, LACZONE, NIE WYBIERANE.
  //
  // Webowe: cytowania ALBO wyniki wyszukiwania. To wyklucza sie WZAJEMNIE
  // i tak zostaje — cytowania znaczą „model naprawde tego uzyl", a pelna lista
  // wynikow jest tylko awaryjna, gdy cytowan nie ma. Pokazanie obu naraz
  // zawyzaloby to, co agent faktycznie przeczytal.
  //
  // Lokalne (ctx.sources): odkladane przez narzedzia wykonywane u nas — dzis
  // przez ollame, po rundzie 7 takze przez narzedzie RAG. Te NIE konkuruja
  // z webowymi: pochodza z innego swiata (fragmenty dokumentow konta wobec
  // stron internetowych) i zadne z nich nie jest „lepszym wariantem" drugiego.
  // Dlatego suma, nie wybor.
  //
  // KOLEJNOSC: najpierw lokalne. Wiedza wlasna konta jest tym, po co agent
  // w ogole ma dokumenty; wynik z sieci jest uzupelnieniem.
  const webowe =
    citedSources.size > 0
      ? Array.from(citedSources.values())
      : Array.from(resultSources.values());

  const lokalne = Array.isArray(ctx.sources) ? ctx.sources : [];
  const sources = [...lokalne, ...webowe];
